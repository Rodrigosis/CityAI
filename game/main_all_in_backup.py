...
class ContentBox:
    ...
    def add_scroll(self, surface, x, y, width, height, content_height, scroll_y):
        ...
        pygame.draw.rect(surface, (150, 150, 150), (x + width - 5, scroll_bar_y, 5, scroll_bar_height),
                         border_radius=5)  # Barra de rolagem
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if windows.is_mouse_over(*player_box[:4]):
                windows.scroll_y_player = windows.handle_scroll(event, windows.scroll_y_player, player_box[4],
                                                                player_box[3])
            elif windows.is_mouse_over(*characters_box[:4]):
                windows.scroll_y_characters = windows.handle_scroll(event, windows.scroll_y_characters,
                                                                    characters_box[4], characters_box[3])
            elif windows.is_mouse_over(*midley_box[:4]):
                windows.scroll_y_midley = windows.handle_scroll(event, windows.scroll_y_midley, midley_box[4],
                                                                midley_box[3])
